...
def identify_obdme_files(work_dir):
    """Identify obdme files and parse their filenames.

    Arguments:

        work_dir (str): Directory to search for obdme files.
    ...
    #
    # These are files named "statrobdme", as generated by mfdn for static
    # densities.
    obdme_files = {}
    ...
    # get filenames for transition densities and extract quantum numbers
    #
    # These are files named "robdme", as generated by mfdn-transitions (or,
    # historically, by mfdn v14 for transition densities).
...
def evaluate_ob_observables(task, postfix=""):
    ...
    # parse obdme filenames
    obdme_files = identify_obdme_files(work_dir)
...
def convert_ob_densities(task, postfix=""):
    """Convert one-body densities to tabular form for interchange with other codes.

    At this stage, it is inconvenient to determine what highest multipolarity
    *might* have been calculated as a byproduct of requested one-body
    observables (if any).  Therefore, this function depends upon an explicit
    specification via the "obdme_multipolarity" task dictionary field, or else
    assumes 0 as the highest available multipolarity.
    ...
    # make conversion output directories
    densities_obme_dir = os.path.join("densities-obme")
    mcscript.utils.mkdir(densities_obme_dir, exist_ok=True)
    densities_dens_dir = os.path.join("densities-dens")
    mcscript.utils.mkdir(densities_dens_dir, exist_ok=True)
    ...
    # sort by sequence number of final state, then sequence number of initial state
    for qn_pair,(filename,code) in sorted(obdme_files.items()):
        ((J_bra, g_bra, n_bra), (J_ket, g_ket, n_ket)) = qn_bra, qn_ket = qn_pair

        # identify target multipolarity range
        obdme_multipolarity = task.get("obdme_multipolarity", 0)
        J0_min, J0_max = abs(int(J_bra-J_ket)), min(abs(int(J_bra+J_ket)), obdme_multipolarity)
        print("State pair {}: multipolarity range {}..{}".format(qn_pair, J0_min, J0_max))
        
        # accumulate obdme-conv input lines
        lines = []
        ...
        # define target obme files
        for J0 in mcscript.utils.value_range(J0_min, J0_max, 1):
            obme_filename = os.path.join(densities_obme_dir, "densities-{:s}-{:s}-k{:d}_obme.dat".format(
                mfdnres.tools.qn_str(qn_bra),
                mfdnres.tools.qn_str(qn_ket),
                J0,
            ))
            lines.append("set-output-filename {:s} {:d}".format(obme_filename, J0))
        ...
        mcscript.control.call(
            [
                environ.shell_filename("obdme-conv")
            ],
            input_lines=lines,
            mode=mcscript.control.CallMode.kSerial
        )

        # convert obme files to tabular form
        for J0 in mcscript.utils.value_range(J0_min, J0_max, 1):
            obme_filename = os.path.join(densities_obme_dir, "densities-{:s}-{:s}-k{:d}_obme.dat".format(
                mfdnres.tools.qn_str(qn_bra),
                mfdnres.tools.qn_str(qn_ket),
            ...
            dens_filename = os.path.join(densities_dens_dir, "densities-{:s}-{:s}-k{:d}_dens.dat".format(
                mfdnres.tools.qn_str(qn_bra),
                mfdnres.tools.qn_str(qn_ket),
                J0,
            ))
    ...
    # copy converted densities out
    out_file_list = glob.glob(os.path.join(densities_dens_dir, "*"))
    mcscript.task.save_results_multi(
        task, out_file_list, descriptor, "dens", command="cp"
    )
...
def run_postprocessor_one_body(task, postfix=""):
    ...
    while db.execute("SELECT * FROM `ob_transitions` WHERE finished is NULL;").fetchone():
        ...
        obdme_multipolarity = task.get("obdme_multipolarity", 0)  # user can request higher multipolarity for obdme beyond that required for operators
