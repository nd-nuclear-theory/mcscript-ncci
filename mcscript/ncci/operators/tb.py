"""tb.py -- define two-body operators for h2mixer input

Two-body sources. A tbme source, as given in "tbme_sources", is defined by the
dictionary mapping:

    id: parameter_dict

Here parameters_dict may possibly contain the following:
...
These correspond to the keywords that can be given to the code h2mixer to
specify how it should construct an operator.  See initial program comments in
h2mixer.cpp.

Two-body operators.  A two-body operator is specified by the tuple:

    (id, qn, coefficient_dictionary)

Here qn is a tuple of (J0,g0,Tz0), and the coefficient_dictionary is of the form
CoefficientDict({"tbme_source_id": coefficient, ...}).  A two-body source id
specified in the dictionary may be the id for an explicitly defined tbme source,
or it may be one of the following special keys:

    + "U[obme_id]": the A-dependent "upgrade" of a one-body source

    + "V[obme_id_a,obme_id_b]": the (symmetric) product of two one-body
        operators

These are shorthands for defining a tbme source with the keyword "operatorU" or
"operatorV", respectively.

Two-body operator sets. Predefined sets of two-body observables can be generated
by adding the following to the list tb_observable_sets:

    + "H-components": Tintr, Tcm, Ncm, VNN, VC
...
def ShellModelHamiltonian(
    ...
    Hmf = mcscript.utils.CoefficientDict(Hmf_unscaled=1/(valence_nucleons-1))
...
def get_tbme_targets(task):
    ...
    if "Hmf" in tb_observable_sets:
        if task["basis_mode"] is modes.BasisMode.kShellModel:
            valence_nucleons = A - sum(task["truncation_parameters"]["mb_core"])
            targets[(0,0,0)]["Hmf"] = mcscript.utils.CoefficientDict(Hmf_unscaled=1/(valence_nucleons-1))
...
def get_tbme_sources(task, targets, postfix):
    ...
    if "Hmf_unscaled" in required_tbme_sources:
        Hmf_filename = task.get("Hmf_file")
        if Hmf_filename is None:
            Hmf_filename = environ.find_interaction_file(
                "{}-U".format(task["interaction"]),
                None,
                None,
            )
        tbme_sources["Hmf_unscaled"] = dict(filename=Hmf_filename)
