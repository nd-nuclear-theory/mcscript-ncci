...
One-body sources.  A one-body source, as given in "obme_sources", is defined
by the dictionary mapping:

    id: parameter_dict

Here parameters_dict must contain exactly one of the following items, which
...
Builtin one-body sources.  Several builtin one-body source ids are provided, for
common one-body operators:

    + kinematic: "identity", "r", "r.r", "ik", "ik.ik", "rtz", "r.rtz", "iktz",
    "ik.iktz"

    + am operators: "j", "j2", "l", "l2", "s", "s2", "ltz", "stz", "sp", "sn",
    "sp2", "sn2"

    + isospin operators: "tz", "t+", "t-", "delta_p", "delta_n" 

    + ladder operators: "c+", "c-"

    + solid harmonics: "r<p>Y<l>", "ik<p>Y<l>", "r<p>Y<l>tz", "ik<p>Y<l>tz"
    (where <p> is a power and <l> is a multipolarity)

These are simply shorthands for pre-defined one-body sources, which otherwise
you would have had to explicitly define, e.g., for "r.r", the implicit
definition is

    ("r.r", {"builtin": "kinematic", "qn": (0,0,0)})

or for the solid harmonic "r2Y2" (i.e., the mass quadrupole operator), the
implicit definition is

    ("r2Y2", {"builtin": "solid-harmonic", "coordinate": "r", "order": 2, "qn": (2,0,0)}),
...
generated by adding the following to the list "ob_observable_sets":
...
- 04/14/25 (mac): Extend module docstring to document builtin one-body source ids.
